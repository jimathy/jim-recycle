		...
		Targets[nameSell] =
			createBoxTarget(
				...
				}, {
				{
					...
					label = Loc[Config.Lan].menu["sell_mats"],
				},
			}, 1.5)
	end
	...
	if data.enter then
		if Config.RecyclingCenter.EnableOpeningHours then
			...
			if (ClockTime >= Config.RecyclingCenter.OpenHour and ClockTime < 24) or (ClockTime <= Config.RecyclingCenter.CloseHour -1 and ClockTime > 0) then
				...
				triggerNotify(nil, Loc[Config.Lan].error["wrong_time"]..Config.RecyclingCenter.OpenHour..":00am"..Loc[Config.Lan].error["till"]..Config.RecyclingCenter.CloseHour..":00pm", "error")
				return
			end
		end
		if Config.RecyclingCenter.PayAtDoor then
			if getPlayer().cash >= Config.RecyclingCenter.PayAtDoor then
				...
				triggerNotify(nil, Loc[Config.Lan].error["no_money"], "error")
				return
			end
		end
...
Recycling.Functions.dutyToggle = function(data)
	if Config.JobRole then
		...
		onDuty = not onDuty
		if onDuty then
			triggerNotify(nil, Loc[Config.Lan].success["on_duty"], 'success')
			Recycling.PickUpPackage.PickRandomEntity(data.Trolly)
		else
			triggerNotify(nil, Loc[Config.Lan].error["off_duty"], 'error')
			Recycling.Functions.EndJob()
		end
	end
end
...
Recycling.Trade.Menu = function(data)
	if not hasItem("recyclablematerial", 1) then
		triggerNotify(nil, Loc[Config.Lan].error["no_mats"], "error")
		return
	end
	...
	openMenu(tradeMenu,{ header = Loc[Config.Lan].menu["mats_trade"], canClose = true, })
end
...
Recycling.Trade.selectMenu = function(data)
	...
	openMenu(tradeMenu, {
		header = Loc[Config.Lan].menu["mats_trade"],
		onBack = function()
